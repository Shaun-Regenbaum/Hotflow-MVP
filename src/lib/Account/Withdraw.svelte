...
<div id="container">
	{#if submitted}
		...
		{:else}
			{#if success}
				<h3>Withdrawal for ${Number(withdrawal.amount / 100).toLocaleString('en', { minimumFractionDigits: 2 })} successfully submitted.</h3>
			{:else}
				<p>{message}</p>
			{/if}
		{/if}
...
<form on:submit|preventDefault={submit_withdrawal}>
	...
	<fieldset>
		...
		<input type="text" id="platform" name="platform" list="platforms" bind:value={withdrawal.platform} />
		<datalist id="platforms">
			<option value="Venmo">
			<option value="Cashapp">
			<option value="Zelle">
			<option value="Paypal">
			<option value="Other (Just Write It Out)">
		</datalist>
	</fieldset>
	<label for="identifier">Id/Username on Platform:</label>
	<input type="text" id="identifier" name="identifier" bind:value={withdrawal.identifier} />
	<label for="email">Email (for if problems arise):</label>
	<input type="email" id="email" name="email" bind:value={withdrawal.email} />
