 ...
 * 3) Perhaps there is a way to just auto-incrememt a value postgresql, dont know how right now.
 *  */

export async function updateBalance(userId: string, amount: number) {
	let balance;

	let { data: profile, error } = await supabase.from('profiles').select('balance').eq('id', userId);
